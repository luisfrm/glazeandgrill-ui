...
export default function ProductDetail({ product, onBack }: ProductDetailProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col lg:flex-row gap-8">
      {/* Image Section */}
      <div className="bg-transparent w-full lg:w-1/2 relative bg-gray-100 h-64 lg:h-auto min-h-42 rounded-lg overflow-hidden">
        <Image
          src={product.image}
          alt={product.name}
          fill
        ...
        />
        {
          onBack && (
            <button
              onClick={onBack}
              className="absolute top-4 left-4 bg-white/80 backdrop-blur-md p-2 rounded-full hover:bg-white transition-colors"
            >
